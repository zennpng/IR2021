...
class Ranking_model(nn.Module):
    ...
    def forward(self, qry_tokens, pos_doc_tokens, neg_doc_tokens):
        ...
        out_qry = self.encoder(qry_embedded)
        out_pos = self.encoder(pos_doc_embedded)
        out_neg = self.encoder(neg_doc_embedded)

        #concat query-positive document and query-negative document
        concat_q_pos_doc = torch.cat((out_qry[0].sum(dim=1), out_pos[0].sum(dim=1)),1)
        concat_q_neg_doc = torch.cat((out_qry[0].sum(dim=1), out_neg[0].sum(dim=1)),1)
